...
import {Header as HeaderProps, Link as SanityLink} from "@/sanity.types";
...
export default function Header(block: HeaderProps) {
  ...
  const [currentMobileView, setCurrentMobileView] = useState<{title: string, items: any[]}>({
    title: "Main Menu",
    items: block.menuItems || NavBarItems
  });
            ...
            className={`
              ${isMobile 
                ...
                : level > 0 
                  ...
                  : ` text-sm font-neulis-sans sm:text-base tracking-[0.5px] leading-5 whitespace-nowrap py-4 relative`
              }
        ...
        return (
          <li key={`${item.title}-${index}`} className="w-full">
            <button 
              ...
              className="font-medium font-neulis-sans text-lg tracking-[0.5px] leading-5 flex items-center gap-1 py-2 w-full justify-between"
                        ...
                        {category.items && category.items
                          ...
                          >
                            <Link
                              ...
                              className="block font-neulis-sans text-sm py-2 pl-4 transition-colors relative rounded-sm"
                              style={{
          ...
          <button 
            ...
            className={`
              ${level > 0 
                ...
                : `font-medium text-sm font-neulis-sans sm:text-base tracking-[0.5px] leading-5 flex items-center gap-1 whitespace-nowrap py-4 relative`
              }
            `}
            style={{ 
            ...
            <div 
              className={`
                ${level > 0 
                  ...
                  : "absolute font-neulis-sans top-full mt-1 left-0 bg-white shadow-md rounded-md py-2 min-w-[180px] z-50 transform transition-all duration-300 ease-in-out"
                }
              `}
              style={{
  ...
  useEffect(() => {
    ...
    setCurrentMobileView({
      title: "Main Menu",
      items: (block.menuItems || NavBarItems).filter((item: any) => 
  ...
  }, [firstPathSegment, block.menuItems]);
      ...
      <header className="fixed z-50 w-full inset-x-0 top-0 bg-white/70 backdrop-blur-lg border-b" style={{ borderColor: borderColor }}>
        <div className="max-w-[1600px] mx-auto">
          <div className="flex items-center justify-between px-4 sm:px-6 md:px-8 lg:px-16 py-2 md:py-4">
            {/* Logo */}
            <Link className="flex items-center gap-2 flex-shrink-0 mr-2 md:mr-4" href="/">
            ...
            <nav className="hidden lg:flex items-center justify-between flex-1">
              <ul role="list" className="flex items-center gap-4 lg:gap-6 xl:gap-8 ml-[54px]">
                ...
                {/* Regular Menu Items */}
                {
                  (block?.menuItems || NavBarItems).map((item, index) => (
              ...
              <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 ml-4 sm:ml-6 lg:ml-8">
                <SecondaryButton href={getLink(firstPathSegment == 'practitioner' ? block.pracPrimaryButtonLink : block.secondaryButtonLink) || '/'}>
                  {firstPathSegment == 'practitioner' ? block.pracPrimaryButton : block.secondaryButton}
                </SecondaryButton>
                <BookingButton>
                  {firstPathSegment == 'practitioner' ? block.pracSecondaryButton : block.primaryButton}
                </BookingButton>
              </div>
            </nav>
          </div>
                    ...
                    <div className="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                      ...
                      {mobileNavStack.length > 0 && (
                        <button 
                          ...
                          className="flex items-center font-neulis-sans font-[500]"
                      ...
                      {currentMobileView.items.map((item, index) => 
                        item.type === "dropdown" ? (
                          <li key={`${item.title}-${index}`} className="w-full">
                            <button 
                              onClick={() => navigateToSubmenu(item)}
                              className="font-neulis-sans font-[500] tracking-[0.5px] leading-5 flex items-center gap-1 py-2 w-full justify-between"
                              style={{ color: healthPurple950 }}
                            >
                              <span>{item.title}</span>
                              <svg 
                          ...
                          <li key={`${item.title}-${index}`}>
                            <Link 
                              ...
                              className="font-neulis-sans font-[500] tracking-[0.5px] leading-5 block py-2"
                      ...
                      {mobileNavStack.length === 0 && (
                        <li className="pt-4 mt-4 border-t" style={{ borderColor: borderColor }}>
                          <div className="flex flex-col gap-4">
                            <SecondaryButton onClick={() => setMobileMenuOpen(false)}>
                              Login
                            </SecondaryButton>
                            <PrimaryButton onClick={() => setMobileMenuOpen(false)}>
                              Book An Appointment
                            </PrimaryButton>
                          </div>
                        </li>
